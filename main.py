...
app = Flask(__name__)
app.secret_key = "sujal_hawk_multi_ist_2026"
...
def start():
    ...
    state["running"] = False
    time.sleep(1)
    state = {"running": True, "sent": 0, "logs": ["MULTI ID + MULTI NC + IST STARTED"], "start_time": time.time()}

    raw_ids = request.form["sessionids"].strip().split("\n")
    cfg["sessionids"] = [s.strip() for s in raw_ids if s.strip()][:5]
    ...
    log(f"STARTED WITH {len(cfg['sessionids'])} ACCOUNTS | Round-Robin + Multi NC")
